...
function theme_wc_get_variations() {
    ...
    if (!isset($_theme_wc_variations)) {
        $print_separator = create_function('', 'echo "%ADD_TO_CART%";');
        add_action('woocommerce_before_add_to_cart_button', $print_separator, 100);
        add_action('woocommerce_after_add_to_cart_button', $print_separator, 1);
        ob_start();
        woocommerce_template_single_add_to_cart();
        $content = ob_get_clean();
        remove_action('woocommerce_before_add_to_cart_button', $print_separator, 100);
        remove_action('woocommerce_after_add_to_cart_button', $print_separator, 1);
...
EOL;
    ...
    if ($variations['show_button']) {
        ?>
        <button onclick="jQuery('<?php echo $context; ?> [class*=bd-productvariations] form').submit()" <?php echo $attributes; ?> class="<?php echo $class; ?> single_add_to_cart_button"<?php echo $additional_style; ?>>
            <?php
...
function theme_change_cart_page_template() {
    if (is_page()) {
        global $post;
        if (function_exists('woocommerce_get_page_id') && woocommerce_get_page_id('cart') == $post->ID && !is_page_template('cart.php')) {
